...
public class HBaseUtils extends BaseUtils {

    private static SystemProps systemProps;

    static {
        systemProps = context.getBean(SystemProps.class);
        String namespace = systemProps.getHBaseNamespace();
        String creator = systemProps.getUser();
        if (!existNamespace(namespace)) {
            createNameSpace(namespace, creator);
        }
    }
    ...
    public static void simpleCreateTable(String tableName, String... columnFamilies) {
        ...
        for (int i = 0; i < cols.length; i++) {
            cols[i] = ColumnFamilyDescriptorBuilder.of(columnFamilies[i]);
        }
    ...
    public static Get createGet(String rowKey, String cfName, String qualifier) {
        Get get = new Get(Bytes.toBytes(rowKey));
        get.addColumn(Bytes.toBytes(cfName), qualifier.getBytes());
        return get;
    }
    ...
    public static Get createMultiVersionGet(String rowKey, String cfName, String qualifier, long from, long to) throws IOException {
        Get get = new Get(Bytes.toBytes(rowKey));
        get.addColumn(cfName.getBytes(), qualifier.getBytes())
